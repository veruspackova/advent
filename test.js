...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const note = document.getElementById("dossierNote");

  const dialog = document.querySelector(".window");
  const closeButton = document.querySelector(".dialog-close");

  // Put the day number into each cell from data-day
  cells.forEach((cell) => {
  ...
  cells.forEach((cell) => {
    ...
    // INITIAL STATE SETUP
    if (isUnavailable) {
      // Unavailable doors: locked, shake if clicked
      ...
      // Available in test mode (not opened and not unavailable)
      ...
      // HOVER → show key (number hidden via CSS .hover-key .cell-number { display:none; })
    ...
    cell.addEventListener("click", () => {
      ...
      // Already opened → just open dialog
      if (cell.classList.contains("opened")) {
        openDialogForDay(currentCellDay);
        return;
      }

      // Available & locked → unlock + then open dialog
      cell.classList.remove("hover-key");
